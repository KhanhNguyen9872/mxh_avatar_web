...
echo'<div class="phdr">Khu Luyện Tập | <a href="/idthienan_ninja">Trở Về Làng</a></div><div class="da"><div class="lucifer">';
$kiemtra = mysql_fetch_array(mysql_query("SELECT * FROM `tuipkm` WHERE `user_id`='".$user_id."'"));
if(empty($kiemtra)){
echo'Ngay cả nhân vật còn không có mà còn bà đặt đi rừng! - <a href="/idthienan_ninja/shop.php">Mua Nhân Vật</a> ';
include'../incfiles/end.php';
exit;
}
$check = mysql_fetch_array(mysql_query("SELECT * FROM `pkmchien` WHERE `id`='".$id."'"));
$check['name'] = addslashes($check['name']);
$check['hp'] = intval($check['hp']);
$check['sm']=intval($check['sm']);
$check['img']=addslashes($check['img']);
if(empty($check['id'])){
echo'Quái hiện không tồn tại <a href="/idthienan_ninja/dirung.php">Chọn Quái Khác</a>';
include'../incfiles/end.php';
exit;
}
$kt = mysql_result(mysql_query("SELECT COUNT(*) FROM `pkmn` WHERE `id_user`='{$user_id}'"), 0);
if($kt==0){
mysql_query("INSERT INTO `pkmn` SET 
`id_user`='".$user_id."',
`name`='".$check['name']."',
`img`='".$check['img']."',
`hp` = '".$check['hp']."',
...
$timepk =time();
if($kt==0){
echo'Đánh Hụt. Vui Lòng <a href="/idthienan_ninja/tc.php?id='.$checkpa['id'].'">Load Lại Trang</a>';
include'../incfiles/end.php';
exit;
}
...
echo'<a href="/idthienan_ninja"><input type="button" value="Trở về Làng"/></a>';
require('../incfiles/end.php');
exit;
}

if($pkmcd['hp']<=0){
echo'Nhân vật kiệt sức <a href="/idthienan_ninja">quay về</a> hồi sức rồi đánh tiếp nhé!';
...
if($ktui['hp']<=0){
echo'Nhân Vật Của Bạn Đã Chết Rồi. Bạn Đã Để Quái Chạy Thoát. 1 Tiếng Sau Quay Lại Giết Nó Nhé! <a href="/idthienan_ninja">Oki^^</a>';
mysql_query("update `users` set `timepkm` = '".time()."' where `id` = '".$user_id."'");
include'../incfiles/end.php';
exit;
}
$checkpa = mysql_fetch_array(mysql_query("SELECT * FROM `pkmn` WHERE `id_user`='".$user_id."'"));
if($id != $checkpa['idpk']){
echo'Đang Đánh Con '.$checkpa['name'].' Sao Nhảy Qua Con Khác Bạn <a href="/idthienan_ninja/dirung.php">Trở Lại!</a>';
include'../incfiles/end.php';
exit;
}
echo'
...
mysql_query("update `pkmn` set `hp` = `hp`-'".$sucdanh."' where `id_user` = '".$user_id."'");
