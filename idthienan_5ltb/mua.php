...
$check = mysql_fetch_array(mysql_query("SELECT * FROM `shoppkm1` WHERE `id`='".$id."'"));
...
if($datauser['vnd']>=$check['gia']){
$time2 = time();
$tuipkmi = mysql_result(mysql_query("SELECT COUNT(*) FROM `tuipkm1` WHERE `user_id`='{$user_id}'"), 0);
if($tuipkmi == 0) {
mysql_query("INSERT INTO `tuipkm1` SET `user_id`='" . $user_id. "',`hp`='" . $check['hp'] . "',`sm`='" . $check['sm']. "',`time`='".$time2."',`img`='" . $check['img'] . "',`name`='" . $check['name'] . "',`hpfull`='" . $check['hp'] . "',`idpkm`='{$id}'");
}
if($tuipkmi > 0) {
@mysql_query("update `tuipkm1` set `hp`='".$check['hp']."',`sm`='".$check['sm']."',`hpfull`='".$check['hp']."',`img`='".$check['img']."',`name`='".$check['name']."' where `id` = '" . $user_id . "',`idpkm`='{$id}'");
}
