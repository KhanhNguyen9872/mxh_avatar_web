...
$check = mysql_fetch_array(mysql_query("SELECT * FROM `shoppkm1` WHERE `id`='1'"));
$check1 = mysql_fetch_array(mysql_query("SELECT * FROM `tuipkm1` WHERE `user_id`='{$user_id}'"));
if($check['check']>0){
...
mysql_query("UPDATE `tuipkm1` SET `exp`=`exp`+'5000',`checklv`='1' WHERE `user_id`='{$user_id}'");
mysql_query("UPDATE `shoppkm1` SET `check`=`check`-'1' WHERE `id`='1'");
