...
$textl = functions::checkout($res['text']).' | '.$vina4u1.''.$vina4u.'8vui.TOP';
}
$top = $res['text'];
$top = functions::checkout($top);
if($res['type']=='t') {
...
-----------------------------------------------------------------
*/
echo '<div class="homeforum"><div class="icon-home"><div class="home">Diễn Đàn</a></div></div></div></div></div>';
...
echo $i % 2 ? '<div class="lucifer">' : '<div><div><div><div class="da"><div class="lucifer">';
...
-----------------------------------------------------------------
*/
echo '<div class="homeforum"><div class="icon-home"><div class="home">Diễn đàn</div></div></div><div><div><div>';
echo'<div class="da">';
...
//
if(($id ==4 ||$id==385) & $rights < 9 ) {
echo '<div class="lucifer"><div id="khung" class="topmenu"><font color="red">Bạn không có quyền đăng bài viết tại chuyên mục này</font></div>';
}else{
...
echo is_integer ( $i / 2 ) ? '<div class="lucifer">' : '<div class="forumlist">';
echo 'Chú ý: ' ;
if ($res['indam'] == 1)
echo'<b> ';
...
if ($res['vip'] == 0)
echo is_integer ( $i / 2 ) ? '<div class="lucifer">' : '<div class="lucifer">' ;
...
echo'' . functions::display_pagination('' . $set['homeurl'] . '/forum/' . $id . '-', $start, $total, $kmess) . '';
echo'</div>';
}
echo '</div>';
break;
...
echo '<div class="news">Bạn không có quyền để Xem topic này.</div>';
require('../incfiles/end.php');
exit;
}
$vipgun=mysql_query("SELECT * FROM `forum` WHERE `type`='r' AND `id`= '".$type1['refid']."'");
...
if ($colmes > $kmess) {
echo '<span style="float:left;"><a href="/index.php"><font color="f8f8ff">Trở lại</font></a></span>' . functions::display_pagination('' . $set['homeurl'] . '/forum/' . $id . '-', $start, $colmes, $kmess) . '';
}
echo'</div>';
...
$postreq = mysql_query("SELECT `forum`.*, `users`.`sex`, `users`.`rights`, `users`.`thank_duoc`, `users`.`lastdate`,`users`.`datereg`,  `users`.`name`
...
$i = 1;
while (($res = mysql_fetch_assoc($req)) !== false) {
echo'<div class="da"><div class="forumtext">';
...
echo'<center><div class="box_bai_gui"><small><b>Bài:</b>' .$res_u['postforum'] .'</small><br/>';
...
<tr><td class="current-blog" rowspan="10" style=""><div class="blog-bg-left">';
...
// Chuc vu MXH VNTEENVIET
$user_rights = array(
3 => '<font color="blue"> - Fmod</font>',
6 => '<font color="#008000"> - Smod</font>',
7 => '<font color="red"> - Admin</font>',
);
...
echo'<span id="'.$res['id'].'" style="color:#' .$colornick['colornick'] . '"><b>'.$user_rights[$res['rights']].'</b><b><span style="float:right;padding-right: 3px;">'.(($dema+$i) != 1 ? '#<b>'.(($dema+$i)-1).'</b>':' ').'</span></b></span>';
//
$sql=mysql_query("select * from `forum_theodoi` where `uid`='$user_id' and `tid`='".$type1['id']."'");
...
echo'<span style="float:right;"><span style="color:#' .$colornick['colornick'] . '"><b>
<a href="'.$home.'/forum/theodoi.php?t='.$id.'"><input type="button" value="B&#7887; theo dõi"/></a>
</b>  </span></span>';
}
}
//
...
echo '<img src="' . $set['homeurl'] . '/files/forum/tailen/' . $fres['filename'] . '" alt="' . $type1['text'] . '" style="word-wrap : break-word;padding: 1px; margin: 4px 4px 0 0; border: 1px solid #d5d5d5;"/></a></p>';
} else {
echo'<div class="menu"><a href="' . $set['homeurl'] . '/files/forum/tailen/' . $fres['filename'] . '"><b>' . $fres['filename'] . '</b></a>';
echo'<br/>';
}
++$i;
}
...
echo'<a href="' . $set['homeurl'] . '/forum/?id='.$id.'&thanks='.$res[id].'&user='.$res[user_id].'&start=0&thank#thanksyou"><img src="/images/like.png" alt="Like" title="Like"/></a>&#160;';
}
if ($user_id && $user_id != $res['user_id']) {
...
if ($res['user_id'] == $datauser['id']||($rights>=3&&$rights>=$res[rights])) {
echo '<a  href="' . $set['homeurl'] . '/forum/' . $res['id'] . '-edit.html"> <img src="/forum/sua.png"/> </a>';
echo '<a href="' . $set['homeurl'] . '/forum/' . $res['id'] . '-del.html"><img src="/forum/xoa.png"/></a>';
}
echo'</div>';
...
echo '<div class="news">';
if ($rights == 3 || $rights >= 6)
echo'<a href="' . $set['homeurl'] . '/forum/' . $res['id'] . '-edit.html"><img src="/forum/sua.png"/></a> <a href="' . $set['homeurl'] . '/forum/' . $res['id'] . '-del.html"><img src="/forum/xoa.png"/></a><a href="/users/profile.php?act=ban&mod=do&user=' . $res['user_id'] . '"><img src="/forum/khoa.png"/></a><a href="/panel/tool.php"><img src="/forum/tool.png"></a>';
if ($rights == 3 || $rights >= 6) {
echo'</div>';
}
*/
...
echo '<textarea rows="' . $set_user['field_h'] . '" name="msg" size="98%"></textarea><br>' .
'<input name="submit" value="Gửi" type="submit" class="nut"/>' .
'<input type="hidden" name="token" value="' . $token . '"/>';
