...
if (isset($_GET['error'])) {
echo '<div class="omenu"><font color="red">Không đủ tiền!!!</font></div>';
}
echo '<div class="phdr">Shop đồ </div>';
?>
<div class="lucifer">
<b>Chọn loại đồ muốn mua : </b><select class="btn btn-dark" name="url"size="1"onChange="window.open(this.options [this.selectedIndex].value,'_top')" style="border: 0;">
<?php
...
@mysql_query("INSERT INTO `khodo` SET
 ...
 `tenvatpham` ='".$post['tenvatpham']."',
 `loai`='".$post['loai']."'");
echo '<div class="list1"><font color="green">Mua thành công vật phẩm: <b>'.$post[tenvatpham].'</b></font></div>';
} else {
header('Location: list.php?error');
}
}
...
echo '<div class="list1"><button class="btn btn-info" name="submit"><i class="fa fa-usd" aria-hidden="true"></i> Mua <i class="fa fa-usd" aria-hidden="true"></i></button></div>';
}
echo '</form>';
if ($total > $kmess) {
echo '<div class="topmenu">' . functions::display_pagination('/shop/list.php?', $start, $total, $kmess) . '</div>';
}
echo '';
